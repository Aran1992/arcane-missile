...
export function resetState() {
  bullets.forEach(b => { b.alive = false; if (gameLayer && b.g.parent) gameLayer.removeChild(b.g); });
  enemies.forEach(e => { e.alive = false; if (gameLayer && e.g.parent) gameLayer.removeChild(e.g); });
  bullets = [];
  enemies = [];
  Object.assign(ctx, {
    hp: 5, maxHP: 5,
