...
export const UPGRADES: UpgradeDef[] = [
  ...
  },
  {
    ...
    unique: true,
    fn: () => {
      ctx.hp = Math.min(ctx.maxHP, ctx.hp + Math.ceil(ctx.maxHP * 0.3));
    },
  },
  {
    id: 'hpup',
...
export function pickUpgrades(): UpgradeDef[] {
  ...
  const weighted = (pool: UpgradeDef[]): UpgradeDef => {
    const t = pool.reduce((s: number, u) => s + RARITY_W[u.rarity], 0);
    let r = Math.random() * t;
    for (const u of pool) {
      r -= RARITY_W[u.rarity];
      if (r <= 0) return u;
  ...
  while (picks.length < 3 && avail.length) {
    ...
    if (picks.includes(p)) {
      const rest = avail.filter((u) => !picks.includes(u));
